...
class Contingency():
    ...
    @classmethod
    def expected_freq(cls, observed):
        ...
        margsums = cls.margins(observed)

        # Create the array of expected frequencies.  The shapes of the
        # marginal sums returned by apply_over_axes() are just what we
        # need for broadcasting in the following product.
        d = observed.ndim
        expected = functools.reduce(np.multiply, margsums) / observed.sum() ** (d - 1)
        return expected

    @classmethod
    def chi2_contingency(cls, observed, correction=True):
        ...
        expected = cls.expected_freq(observed)
