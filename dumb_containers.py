...
def woe(x, y, bin=5, auto=True, toplot=True, title=None):
    if type(bin) != int:
        auto = False
    ...
    if toplot:
        ...
        if title is not None:
            plt.title(title)
        ax2 = plt.twinx()
        plt.plot(np.arange(1,len(binx)+1), woes, '.-k', linewidth=2, markersize=10)
        plt.xticks(np.arange(1, len(binx)+1), bins2range(bins))
        for i, j in zip(np.arange(len(x))+1, woes):
...
def main_get_numeric_ref_table(df,var,tgt,max_bins):
    ...
    if len(df[var].unique()) < max_bins:
        ...
        # for i in np.arange(max_bins+1):
            ...
            # except:
                # pass
        q = df.ix[~df[var].isnull(), [var]].quantile(1.*np.arange(max_bins+1)/max_bins)
        q[var] = q[var].map(lambda x: round(x,5))
        q = q.drop_duplicates()
    ...
    col_t = [c for c in df.columns if c != var and c != var+'_bin' and c != tgt][0]
    ...
    while ds['pop'].min() < pop_cut and ((ds.shape[0] > 2) or (ds[[0, 1]]==0).any().any()):
        # calculate chisquare statistic
        chisq = []
        for i in range(ds.shape[0]-1):
            chisq.append(cy.chi2_contingency(ds.irow([i,i+1])[[0,1]])[0])  
...
def main_get_numeric_ref_table_fr_bins(df, var, tgt, bins, toplot=True, title=None):
    start_time = datetime.now()
    uvbucket = bins
    ...
    if toplot:
        if (ds['ref_table'] == '-inf_-inf').any():
            # -inf_-inf will be the first item
            ds_plt = ds.copy()
            ds_plt['i'] = range(ds_plt.shape[0])
            ds_plt.ix[0, 'i'] = ds_plt.shape[0]
            ds_plt['i'] = ds_plt['i'] - 1
            ds_plt = ds_plt.set_index('i', drop=True).sort_index()
        else:
            ds_plt = ds
            
        plt.bar(np.arange(1, ds_plt.shape[0]+1), ds_plt['pop'])
        if (ds_plt['ref_table'] == '-inf_-inf').any():
            plt.bar(ds_plt.shape[0], ds_plt.ix[ds_plt['ref_table'] == '-inf_-inf', 'pop'], alpha=.8, color='g')
            
        plt.bar(np.arange(1, ds_plt.shape[0]+1), ds_plt[1], color='r')
        if title is not None:
            plt.title(title)
        ax2 = plt.twinx()
        plt.plot(np.arange(1, ds_plt.shape[0]+1), ds_plt['{}_nwoe'.format(var)], '.-k', linewidth=2, markersize=10)
        
        def get_bins_xtick(bins):
            '''
            Change
            0    [-inf, 0.55]
            1    [0.55, 2.43]
            ...
            3      [inf, inf]
            to
            ['[-inf, 0.55)', '[0.55, 2.43)', '[2.43, inf)', 'np.nan']
            '''
            xtick = []
            for i in bins.index:
                if bins.iloc[i][0] == '-inf' and bins.iloc[i][0] == '-inf':
                    xtick.append('np.nan')
                    continue
                xtick.append('[{}, {})'.format(bins.iloc[i][0], bins.iloc[i][1]))
            return xtick
            
        plt.xticks(np.arange(1, ds_plt.shape[0]+1), get_bins_xtick(ds_plt['bin']))
        for i, j in zip(np.arange(ds_plt.shape[0])+1, ds_plt['{}_nwoe'.format(var)]):
            ax2.annotate(str(round(j, 2)), xy=(i, j), va="center", ha="left",
                         bbox=dict(boxstyle="round", fc="w"))
...
def update_numeric_woe_info(ref_table, var, b_stat,
    ...
    df_ref_table_tmp = pd.DataFrame.from_dict(ref_table, orient='index').reset_index().rename(columns={'index': 'Var_Value', 0: 'Ref_Value'})
...
def calc_nominal_woe(df, var, tgt):
    ...
    is_small = ((ds[0] + ds[1]) / df.shape[0]) < 0.001
